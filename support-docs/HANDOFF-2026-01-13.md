...
| Phase | Task | Status |
|-------|------|--------|
| 1 | Create Mux Livestream from Settings | ✅ Done |
| 2 | Variable stream count (1-4) | ✅ Done |
| 3 | Verify blob isolation | ✅ Done |
| 4 | Reset dashboard to 0 streams | ✅ Done |
| 5 | **Create 4 Yabun Festival streams** | **👈 NEXT** |
...
2. Click Settings → Enter producer password (`Live2Stream`)
...
│                                                          │
│  ┌──────────────────────────────────────────────────┐   │
│  │           LXC Container: play-staging             │   │
│  │                                                   │   │
│  │   Node.js + Netlify CLI                          │   │
│  │   └── netlify dev (serves site locally)          │   │
│  │                                                   │   │
│  │   Git clone of Play repo                         │   │
│  │   Environment variables configured               │   │
│  │                                                   │   │
│  │   Internal URL: http://play-staging.local:8888   │   │
│  └──────────────────────────────────────────────────┘   │
│                                                          │
│  Cloudflare Tunnel (optional):                          │
│  └── play-staging.mediahouse.com.au → localhost:8888    │
│                                                          │
└─────────────────────────────────────────────────────────┘
```
...
1. **Get stream names from Scott** for Yabun Festival 2026
2. **Create 4 streams** via Settings UI on staging
3. **Test with real encoders:**
   - OBS Studio (software encoder)
   - Blackmagic Web Presenter (hardware encoder)
4. **Verify webhooks** are updating status correctly
5. **Test break mode** on each stream
6. **Final QA** then merge to main
